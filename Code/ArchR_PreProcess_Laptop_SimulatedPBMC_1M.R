#Preprocessing of Large PBMC 1M
#04/22/20
...
dir.create("/Volumes/Extreme SSD/Analyze_PBMC_1M/")
setwd("/Volumes/Extreme SSD/Analyze_PBMC_1M/")

inputFiles <- getInputFiles("../PBMC_1M")
addArchRGenome("hg19")
...
tstart4 <- Sys.time()
proj <- addClusters(proj, sampleCells = 50000)
saveTime4 <- Sys.time() - tstart4
cat(utils:::capture.output(saveTime4), file = "Save-Time-Clustering.txt")

tstart5 <- Sys.time()
proj <- addUMAP(proj, minDist = 0.1, nNeighbors = 40, sampleCells = 100000, name = "UMAP", force = TRUE)
saveTime5 <- Sys.time() - tstart5
cat(utils:::capture.output(saveTime5), file = "Save-Time-UMAP.txt")

save.image("Save-Analysis-PBMC-1M.Rdata", compress = FALSE)
